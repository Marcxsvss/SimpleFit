...
public class UsuariosJpaController implements Serializable {
    ...
    public void addUsuarioRutina(Usuarios usuarios, Rutinas rutina) throws PreexistingEntityException, Exception {
        ...
        try {
            ...
            // Adjuntar la colección de rutinas del usuario a la entidad gestionada
            if (usuarios.getRutinasCollection() == null) {
                usuarios.setRutinasCollection(new ArrayList<Rutinas>());
            }

            // Adjuntar rutina a la colección del usuario
            usuarios.getRutinasCollection().add(rutina);

            // Asegurar que la relación bidireccional esté establecida
            if (rutina.getUsuariosCollection() == null) {
                rutina.setUsuariosCollection(new ArrayList<Usuarios>());
            }
            rutina.getUsuariosCollection().add(usuarios);

            // Persistir la rutina si es nueva
            if (em.find(Rutinas.class, rutina.getRutinaid()) == null) {
                em.persist(rutina);
            } else {
                em.merge(rutina);
            }

            // Merge del usuario para asegurarse que los cambios sean persistidos
            em.merge(usuarios);
            ...
            if (findUsuarios(usuarios.getEmail()) == null) {
                throw new PreexistingEntityException("Usuarios " + usuarios + " doesn't exists.", ex);
            }
            throw ex;
        } finally {
            if (em != null) {
    ...
    public void destroyRutina(Usuarios usuarios, Rutinas rutina) throws NonexistentEntityException, PreexistingEntityException {
        EntityManager em = null;
        ...
        try {
            ...
            // Eliminar rutina a la colección del usuario
            usuarios.getRutinasCollection().remove(rutina);

            // Asegurar que la relación bidireccional esté establecida
            if (rutina.getUsuariosCollection() == null) {
                rutina.setUsuariosCollection(new ArrayList<Usuarios>());
            }
            //Eliminar usuario a la coleccion de la rutina
            rutina.getUsuariosCollection().remove(usuarios);
