...
class UsuarioServiceImplementation @Inject constructor(
    ...
    suspend fun comprobarEmail(email: String): UsuarioApi {
        val mensajeError = "No se han podido obtener el usuario con email = $email"
        try {
            val response = usuarioService.comprobar(email)
            if (response.isSuccessful) {
                Log.d(logTag, response.toString())
                val dato = response.body()
                return dato ?: throw ApiServicesException("No hay datos")
    ...
    suspend fun rutinas(email : String): List<RutinasApi> {
        val mensajeError = "No se han podido obtener la lista de rutinas"
        try {
            val response = usuarioService.rutinas(email)
            if (response.isSuccessful) {
                Log.d(logTag, response.toString())
                val dato = response.body()
                return dato ?: throw ApiServicesException("No hay datos")
    ...
    suspend fun insertRutina(email : String, rutinaid : Int) {
        val mensajeError ="No se ha podido añadir el usuario"
        try {
            val response = usuarioService.insertRutina(email,rutinaid)
            if (response.isSuccessful) {
                Log.d(logTag, response.toString())
                // Aquí response.body() es un objeto de tipo RespuestaApi
                // que simplemente logeamos si no es null.
    ...
    suspend fun delete(email : String, rutinaid : Int) {
        ...
        try {
            val response = usuarioService.delete(email,rutinaid)
            if (response.isSuccessful) {
                Log.d(logTag, response.toString())
                Log.d(logTag, response.body()?.toString() ?: "No hay respuesta")
            } else {
