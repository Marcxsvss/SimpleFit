...
fun SimpleFitScreen() {
    ...
    var verNavegacion = derivedStateOf {
        iOpcionNevagacionSeleccionada.value != 3 && iOpcionNevagacionSeleccionada.value != 4 && iOpcionNevagacionSeleccionada.value != 5
    }
    ...
    Scaffold(
        ...
        bottomBar = {
            if (verNavegacion.value)
                NavBar(
                    indexScreenState = iOpcionNevagacionSeleccionada.value,
                    onNavigateToHome = { navController.navigateToHome() },
                    onNavigateToProfile = { navController.navigateToProfile()},
                    onNavigateToList  = {navController.navigateToRoutines()})
