...
public class UsersJpaController implements Serializable {
    ...
    public void edit(Users users) throws NonexistentEntityException, Exception {
        ...
        try {
            ...
            if (msg == null || msg.length() == 0) {
                Integer dni = users.getDni();
                if (findUsers(dni) == null) {
                    throw new NonexistentEntityException("The users with id " + dni + " no longer exists.");
                }
            }
            throw ex;
        } finally {
    ...
    public Users findUsers(Integer dni) {
        EntityManager em = getEntityManager();
        try {
            return em.find(Users.class, dni);
        } finally {
            em.close();
        }
    }
